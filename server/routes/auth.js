...
router.post('/login', loginLimiter, async (req, res) => {
  try {
    ...
    const user = { id: row.id, name: row.name, role: row.role, acesso_minha_nota: !!row.acesso_minha_nota, acesso_legalizacao: !!row.acesso_legalizacao };
    const accessToken = auth.signAccess(user);
    const { token: refreshToken } = await auth.issueRefreshToken(row.id);
    return res.json({ token: accessToken, refreshToken, user: { id: row.id, name: row.name, email: row.email, role: row.role, acesso_minha_nota: !!row.acesso_minha_nota, acesso_legalizacao: !!row.acesso_legalizacao } });
  } catch (err) {
...
router.post('/refresh', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT rt.*, u.name, u.role, u.active, u.acesso_minha_nota, u.acesso_legalizacao
    ...
    const user = { id: row.user_id, name: row.name, role: row.role, acesso_minha_nota: !!row.acesso_minha_nota, acesso_legalizacao: !!row.acesso_legalizacao };
    const accessToken = auth.signAccess(user);
