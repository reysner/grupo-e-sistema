...
/**
 * GET /api/data/gam/simulacao-velocidade-media?mes= — TEMPORÁRIO, só pra
 * avaliação (pedido do Reysner, 28/08/2026): mostra lado a lado a nota
 * final REAL de cada colaborador (velocidade embutida por ticket, como é
 * hoje) contra uma nota HIPOTÉTICA onde a velocidade de quem
 * recebeu/atendeu também vira média mensal separada (como já é pra quem
 * só transferiu). Não grava nada, só compara.
 */
router.get('/gam/simulacao-velocidade-media', requireAdmin, async (req, res) => {
  try {
    await ensurePontuacaoSchema(pool);
    const { mes } = req.query;
    if (!mes || !/^\d{4}-\d{2}$/.test(mes)) return res.status(400).json({ error: 'Informe "mes" no formato AAAA-MM.' });
    ...
    for (const c of colaboradores) {
      ...
      if (c.aplica_regra_aceite) {
        const { rows: aceiteRows } = await pool.query(
          `SELECT p.ajuste_aceite FROM gam_tickets_pontos p
           LEFT JOIN gam_aceite_revisoes ar ON ar.ticket_id = p.ticket_id AND ar.papel = p.papel
           WHERE p.mes = $1 AND p.analista_id = $2 AND p.papel IN ('transferiu','unico')
             AND p.ajuste_aceite IS NOT NULL AND COALESCE(ar.status, 'pendente') != 'indevida'`,
          [mes, c.zappy_user_id]
        );
        if (aceiteRows.length) bonusAceite = aceiteRows.reduce((s, r) => s + parseFloat(r.ajuste_aceite), 0) / aceiteRows.length;
      }

      const { rows: finalizarRowsRaw } = await pool.query(
        `SELECT p.ajuste_finalizar, COALESCE(fr.status, 'pendente') AS finalizar_status
         FROM gam_tickets_pontos p
         JOIN cs_tickets t ON t.id = p.ticket_id
         LEFT JOIN gam_finalizar_revisoes fr ON fr.ticket_id = p.ticket_id AND fr.papel = p.papel
         WHERE p.mes = $1 AND p.analista_id = $2 AND p.papel IN ('recebeu','unico')
      ...
      const finalizarValidos = finalizarRowsRaw.filter(r => r.finalizar_status !== 'indevida');
      const bonusFinalizar = finalizarValidos.length
        ? finalizarValidos.reduce((s, r) => s + parseFloat(r.ajuste_finalizar), 0) / finalizarValidos.length
        : 0;

      // ── REAL (como é hoje): velocidade embutida na nota_final por ticket ──
      const somaBaseReal = notasRows.reduce((s, r) => {
        if (r.vel_status === 'indevida') {
          return s + clamp(parseFloat(r.nota_cliente) + 0 + parseFloat(r.ajuste_reabertura), 0, 5);
        }
        return s + parseFloat(r.nota_final);
      }, 0);
      const mediaBaseReal = somaBaseReal / notasRows.length;
      const notaReal = Number(Math.max(0, Math.min(5, mediaBaseReal + bonusTransferencia + bonusAceite + bonusFinalizar)).toFixed(4));

      // ── HIPOTÉTICO: velocidade de quem recebeu/atendeu também vira média
      // mensal separada — indevida EXCLUI o ticket da média (como as outras
      // métricas já fazem), em vez de só zerar o desconto naquele ticket.
      const mediaBaseSemVel = notasRows.reduce((s, r) => s + parseFloat(r.nota_cliente), 0) / notasRows.length;
      const velValidos = notasRows.filter(r => r.vel_status !== 'indevida');
      const bonusVelocidade = velValidos.length
        ? velValidos.reduce((s, r) => s + parseFloat(r.ajuste_velocidade), 0) / velValidos.length
        : 0;
      const notaHipotetica = Number(Math.max(0, Math.min(5,
        mediaBaseSemVel + bonusVelocidade + bonusTransferencia + bonusAceite + bonusFinalizar
      )).toFixed(4));

      resultados.push({
        nome: c.nome, avaliacoes: notasRows.length,
        notaReal, notaHipotetica, diferenca: Number((notaHipotetica - notaReal).toFixed(4)),
      });
    }
    res.json({ mes, resultados });
  } catch (err) {
    console.error('[gam] simulacao-velocidade-media falhou:', err);
    res.status(500).json({ error: err.message || 'Erro ao simular.' });
  }
});
